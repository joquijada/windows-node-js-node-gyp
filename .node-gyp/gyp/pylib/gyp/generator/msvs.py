...
def _FixPath(path):
  ...
  if gyp.common.IsVerbose():
    print "CUSTOM: Path AFTER normalizaton is " + path
  if path and path[-1] == '\\':
    path = path[:-1]
...
def _ChangeToAbsolutePath(path):
  ...
  if gyp.common.IsVerbose():
    print "CUSTOM: path BEFORE abs path fix is " + path
  path = path.replace('..', '')
  cwd = os.getcwd().replace('/cygdrive/c', 'c:')
  if gyp.common.IsVerbose():
    print "CUSTOM: CWD is " + cwd
  path = cwd + path
  
  if gyp.common.IsVerbose():
    print "CUSTOM: path AFTER abs path fix is " + path
  return path
...
def _GenerateNativeRulesForMSVS(p, rules, output_dir, spec, options):
  ...
  if gyp.common.IsVerbose():
    print "CUSTOM: rules_filename is " + rules_filename
...
def _GetLibraries(spec):
  ...
  for entry in reversed(libraries):
    ...
    if gyp.common.IsVerbose():
      print "CUSTOM: Processed lib entry " + entry 
...
def _GetMSVSAttributes(spec, config, config_type):
  ...
  if gyp.common.IsVerbose():                              
    print "CUSTOM: output_dir is " + output_dir
  prepared_attrs['OutputDirectory'] = _FixPath(output_dir) + '\\'
  if gyp.common.IsVerbose():
    print "CUSTOM: in _GetMSVSAttributes prepared_attrs['OutputDirectory'] is " + prepared_attrs['OutputDirectory']
...
def _CreateProjectObjects(target_list, target_dicts, options, msvs_version):
  ...
  for qualified_target in target_list:
    ...
    build_file = gyp.common.BuildFile(qualified_target)
    if gyp.common.IsVerbose():
      print "CUSTOM: build_file is " + build_file
      print "CUSTOM: proj_path is " + proj_path
...
def _ConvertMSVSBuildAttributes(spec, config, build_file):
  ...
  for a in msvs_attributes:
    if a in ['IntermediateDirectory', 'OutputDirectory']:
      ...
      if gyp.common.IsVerbose():
        print "CUSTOM: Calculated directory is " + directory
...
def _GetMSBuildAttributes(spec, config, build_file):
  ...
  if msbuild_tool:
    ...
    if out_file:
      if gyp.common.IsVerbose():
        print "CUSTOM: TargetPath BEFORE fixed " + out_file
      msbuild_attributes['TargetPath'] = _FixPath(out_file)
      if gyp.common.IsVerbose():
        print "CUSTOM: TargetPath AFTER fixed " + msbuild_attributes['TargetPath']
...
def _GetMSBuildConfigurationGlobalProperties(spec, configurations, build_file):
  ...
  for (name, configuration) in sorted(configurations.iteritems()):
    ...
    if gyp.common.IsVerbose():
      print "CUSTOM: OutDir is " + attributes['OutputDirectory']
